<div>
    <div class="mb-4 flex flex-wrap items-end gap-3">
        <div>
            <label class="mb-1 block font-['JetBrains_Mono'] text-[10px] font-medium uppercase tracking-wider text-[#94A3B8]">Tipe Alert</label>
            <select wire:model.live="alertType" class="rounded-lg border border-slate-700 bg-[#0F172A] px-3 py-2 text-xs text-[#F1F5F9] outline-none focus:border-blue-500/50">
        ...
        <button wire:click="$set('alertType', '')" class="rounded-lg border border-slate-700 px-3 py-2 text-xs text-[#94A3B8] transition-colors hover:text-[#F1F5F9]">Reset</button>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b border-slate-700/30 transition-colors hover:bg-slate-700/20 {{ !$alert->is_acknowledged && $alert->alert_type === 'DANGER' ? 'bg-red-500/5' : '' }}">
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="inline-block rounded-full px-2.5 py-0.5 font-['JetBrains_Mono'] text-[10px] {{ $alert->alert_type === 'DANGER' ? 'animate-pulse bg-red-500/30 text-red-400' : ($alert->alert_type === 'DEVICE_OFFLINE' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-green-500/20 text-green-400') }}">
                            {{ $alert->alert_type }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-center font-['JetBrains_Mono'] text-xs text-[#94A3B8]">{{ $alert->sensor_position ?? '-' }}</td>
                    <td class="px-4 py-3 text-right font-['JetBrains_Mono'] text-sm">{{ $alert->distance_value ? number_format($alert->distance_value, 1) . ' cm' : '-' }}</td>
                    <td class="px-4 py-3 text-center">
                    ...
                    <td class="px-4 py-3 text-center">
                        @if(!$alert->is_acknowledged)
                            <button wire:click="acknowledge('{{ $alert->id }}')" class="rounded px-2 py-1 text-xs text-blue-400 hover:text-blue-300">Tandai</button>
                        @else
